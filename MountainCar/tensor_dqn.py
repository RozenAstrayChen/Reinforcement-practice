...
class DeepQNetwork:
    ...
    def __init__(self, n_actions, n_features, learning_rate =0.01, reward_decay= 0.9, e_greedy=0.9, replace_target_iter=300, memory_size=500, batch_size=32, e_greedy_increment=None, output_graph=False):
        ...
        t_params = tf.get_collection('target_net_parans') # extract target_net params
        e_params = tf.get_collection('eval_net_params') # extract eval_net params 
    ...
    def learn(self):
        ...
        if self.memory_counter > self.memory_size:
            sampe_index = np.random.choice(self.memory_size, size=self.batch_size)
        else:
            sampe_index = np.random.choice(self.memory_counter, size=self.batch_size)
